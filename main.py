...
(WAIT_LINK, WAIT_QUANTITY, WAIT_DEPOSIT_AMOUNT, WAIT_DEPOSIT_PROOF, WAIT_BROADCAST) = range(5)
...
async def show_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        f"💳 *رصيدك الحالي*\n\n`{bal:.4f} $`\n\nيمكنك شحن رصيدك عبر زر «➕ شحن رصيد».",
        reply_markup=back_btn("main_menu"), parse_mode="Markdown"
    )
...
async def show_platforms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("🌐 *اختر المنصة:*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def show_services(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not plat:
        await q.answer("منصة غير موجودة!", show_alert=True); return
    context.user_data["platform"] = platform_key
    keyboard = []
    for svc in plat["services"]:
        keyboard.append([InlineKeyboardButton(f"{svc['name']} — {svc['price_per_1k']}$ / 1000", callback_data=f"svc_{svc['id']}")])
    keyboard.append([InlineKeyboardButton("🔙 رجوع", callback_data="platforms")])
    await q.edit_message_text(f"{plat['name']} — *اختر الخدمة:*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

async def select_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    svc = find_service(q.data.replace("svc_", ""))
    if not svc:
        await q.answer("خدمة غير موجودة!", show_alert=True); return
    context.user_data["service"] = svc
    await q.edit_message_text(
        f"✅ *{svc['name']}*\n\n💰 السعر: `{svc['price_per_1k']}$` لكل 1000\n"
        f"📊 الحد الأدنى: `{svc['min']}` | الحد الأقصى: `{svc['max']}`\n\n🔗 أرسل *رابط* الصفحة:",
        reply_markup=back_btn(f"plat_{context.user_data.get('platform', 'instagram')}"), parse_mode="Markdown"
    )
    return WAIT_LINK
...
async def receive_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔢 أرسل *الكمية* المطلوبة:\n(من `{svc['min']}` إلى `{svc['max']}`)", parse_mode="Markdown"
    )
...
async def receive_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("❌ أرسل رقماً صحيحاً فقط"); return WAIT_QUANTITY
    svc = context.user_data["service"]
    link = context.user_data["link"]
    user_id = update.effective_user.id
    if qty < svc["min"] or qty > svc["max"]:
        await update.message.reply_text(f"❌ الكمية خارج النطاق!\nمن `{svc['min']}` إلى `{svc['max']}`", parse_mode="Markdown")
        return WAIT_QUANTITY
    ...
    await update.message.reply_text(
        ...
        reply_markup=keyboard if balance >= price else back_btn("main_menu"), parse_mode="Markdown"
    )
...
async def confirm_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all([svc, link, qty, price]):
        await q.edit_message_text("❌ حدث خطأ، حاول مجدداً."); return
    ...
    try:
        await q.get_bot().send_message(ADMIN_ID,
            f"🆕 *طلب جديد #{order_id}*\n👤 المستخدم: `{user_id}`\n🔧 الخدمة: {svc['name']}\n"
            f"🔗 الرابط: {link}\n🔢 الكمية: {qty}\n💰 السعر: {price:.4f}$\n📡 SMM ID: `{smm_id}`",
            parse_mode="Markdown")
    except Exception:
...
async def my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not orders:
        await q.edit_message_text("📦 لا يوجد لديك طلبات حتى الآن.", reply_markup=back_btn("main_menu")); return
    STATUS_EMOJI = {"pending": "⏳", "processing": "🔄", "completed": "✅", "partial": "⚠️", "cancelled": "❌"}
    lines = ["📦 *آخر طلباتك:*\n"]
    for o in orders:
        oid, sname, link, qty, price, status, created = o
        lines.append(f"{STATUS_EMOJI.get(status,'❓')} *#{oid}* — {sname}\n   🔢 {qty} | 💰 {price:.4f}$ | {status}\n   📅 {created[:10]}\n")
    await q.edit_message_text("\n".join(lines), reply_markup=back_btn("main_menu"), parse_mode="Markdown")
...
async def add_funds(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("➕ *شحن الرصيد*\n\nاختر طريقة الدفع:", reply_markup=keyboard, parse_mode="Markdown")
...
async def pay_mastercard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        f"💳 *الدفع عبر ماستر كارد*\n\nرقم البطاقة:\n`{MASTERCARD_NUMBER}`\n\n⚠️ الحد الأدنى: `{MIN_DEPOSIT}$`\n\nبعد التحويل أرسل *المبلغ* الذي دفعته:",
        reply_markup=back_btn("add_funds"), parse_mode="Markdown"
    )
...
async def receive_deposit_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("❌ أرسل رقماً صحيحاً (مثال: 10)"); return WAIT_DEPOSIT_AMOUNT
    if amount < MIN_DEPOSIT:
        await update.message.reply_text(f"❌ الحد الأدنى للشحن هو {MIN_DEPOSIT}$"); return WAIT_DEPOSIT_AMOUNT
...
async def receive_deposit_proof(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    proof = update.message.photo[-1].file_id if update.message.photo else (update.message.text or "no_proof")
    ...
    msg = f"💰 *طلب شحن جديد #{dep_id}*\n\n👤 المستخدم: `{user_id}`\n💵 المبلغ: `{amount}$`\n💳 الطريقة: {method}"
    try:
        if update.message.photo:
            await update.get_bot().send_photo(ADMIN_ID, photo=proof, caption=msg, reply_markup=keyboard, parse_mode="Markdown")
        else:
            await update.get_bot().send_message(ADMIN_ID, msg + f"\n📄 الإيصال: {proof}", reply_markup=keyboard, parse_mode="Markdown")
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"📊 *إحصائيات البوت*\n\n👥 المستخدمون: `{len(users)}`\n💰 إجمالي الأرصدة: `{sum(u[3] for u in users):.4f}$`",
        parse_mode="Markdown"
    )
